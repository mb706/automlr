...
expect_warning(allwrappers <- bl(w1, w2, w1r, w2r, test1, test2), "is marked 'inject' and has type 'fix';", all=TRUE)
awPS = getParamSet(allwrappers)
expect_set_equal(getParamIds(awPS),c("selected.learner", "test1.int1", "test2.int1", "w1.spare1",
                                     "w1r.spare1", "w2.spare1", "w2.spare2", "w2r.spare2", "automlr.wrappersetup"))
