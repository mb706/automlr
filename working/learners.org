...
**** Variable Parameters:
   ...
   - lambda :: .001..100: regularization parameter; req: penalty %in% c("adaptive.lasso", "ao", "bridge", "genet", "lasso", "oscar", "penalreg", "ridge", "scad")
   - gamma :: .001..100: regularization parameter; req: penalty %in% c("ao", "bridge", "genet", "weighted.fusion")
   - alpha :: .001..100: regularization parameter; req: penalty == "genet"
   - c :: .001..100: regularization parameter; req: penalty == "oscar"
   - a :: .001..100: regularization parameter; req: penalty == "scad"
   - lambda1 :: .001..100: regularization parameter; req: penalty %in% c("enet", "fused.lasso", "icb", "licb", "weighted.fusion")
   - lambda2 :: .001..100: regularization parameter; req: penalty %in% c("enet", "fused.lasso", "icb", "licb", "weighted.fusion")
...
**** Variable Parameters:
   ...
   - cp.type :: bic, aic
**** Changed (fixed) Parameters:
   - cp :: NULL: must be missing, so that cp.type has an effect
** discriminant analysis
*** classif.lda
- Linear discriminant analysis
**** Variable Parameters:
   - method :: moment, mle, mve, t: {da.method}
   - nu :: int 2..64 exp: {da.nu} t degrees of freedom, req: method=='t'
...
**** Variable Parameters:
   ...
   - criterion :: missclassification, deviance: optimization crit for mda init.
**** Default Parameters:
...
**** Variable Parameters:
   ...
   - mh_prob_steps :: MANUAL will be normalized. we use different parameters: a: 0..1, b: 0..1, where mh_prob_steps = ((1-a)*b, (1-a)*(1-b), a)
**** Default Parameters:
   - num_burn_in :: 250: number of trees to use as burn-in
   - num_iterations_after_burn_in :: 1000: number of MCMC samples
   - q :: NULL: not used for classification
   - nu :: NULL: not used for classification
...
**** Variable Parameters:
   ...
   - evenCuts :: TRUE, FALSE
   - subsetSizesIsNull :: TRUE, FALSE: my dummy variable
...
- random forest + pca
**** Variable Parameters:
   - Kinverse :: int 3..40: number of variables per subset. Need to implement K = round(ncol(x) / Kinverse, 0).
...
**** Changed (fixed) Parameters:
   - xval :: 0: no crossvalidation
*** classif.bst
- Gradient Boosting, seems like this wouldn't be adding anything so I'll skip it for now
*** classif.gbm
- "Generalized Boosted Regression Modeling"
**** Variable Parameters:
   - distribution :: bernoulli, adaboost, gaussian, laplace, huberized, multinomial, pooisson, pairwise
...
**** Default Parameters:
   ...
   - objective :: NULL: is supplied by learner function
   - eval_metrix :: NULL: use default
   - maximize :: TRUE: does not matter since early.stop.round is NULL.
**** Changed (fixed) Parameters:
   - nthread :: 1: only one thread
   - verbose :: 0: stay silent
   - print.every.n :: 0: stay silent
   - early.stop.round :: NULL
   - missing :: NULL: we don't have a missing indicator.
** SVM
*** classif.dcSVM
...
**** Variable Parameters:
   ...
   - cluster.method :: kmeans, kernkmeans, mlKmeans: {svm.cluster} clustering algorithm
**** Default Parameters:
...
**** Default Parameters:
   - tol :: .001: termination criterion
   - fit :: NULL: TODO is named 'fitted' in mlr, which is a BUG
**** Changed (fixed) Parameters:
   - scaled :: FALSE: we do that ourselves
*** classif.svm
**** Variable Parameters:
...
** Neural Nets
*** classif.avNNet
- seems to be bagging using nnet; ignoring this for the time being.
*** classif.dbnDNN
**** Variable Parameters:
   ...
   - cd :: 1, 2, 3: boltzmann machine init rounds
**** Default Parameters:
   - visible_dropout :: 0
*** classif.mlp
- Multilayer perceptron
- Skipping this for now since we have a few NNs already
*** classif.multinom
...
**** Variable Parameters:
   ...
   - hidde :: 3..100 exp: len(5) {nn.h5} DUMMY req: numlayers==5
   - hidd :: 3..100 exp: len(7) {nn.h7} DUMMY req: numlayers==7
   - threshold :: .1...0001 exp: stopping criterion
...
**** Variable Parameters:
   ...
   - alpha :: MANUAL [0.1..0.02; 0.02..0.001] exp: {koho.alpha} learning rate
...
**** Default Parameters:
   ...
   - toroidal :: TRUE: edges of map are joined
**** Changed (fixed) Parameters:
   - contin :: FALSE: indicate that we are dealing with categories
   - keep.data :: FALSE
   - model :: FALSE: TODO: The function trainLearner.classif.binomial sets this to FALSE, the getParamSet says this defaults to TRUE. I guess FALSE it is...
*** classif.lvq1
...
**** Variable Parameters:
   ...
   - n.hood :: circular, square: {koho.shape} shape of neighbourhood
**** Default Parameters:
   - radius :: 0: neighbourhood radius; not going to touch this for now
   - toroidal :: TRUE: edges of map are joined
**** Changed (fixed) Parameters:
   - contin :: FALSE: indicate that we are dealing with categories
