...
- VERSION{< 2.10} only in clude for mlr versions before 2.10. possible comparators are <, >, <=, >=
* TODO:
- [ ] remove ONNA
...
**** Variable Parameters:
   ...
   - crossval :: TRUE, FALSE: req: fold > 1
   - crossval.AMLRFIX1 :: FALSE: req: fold == 1
...
**** Variable Parameters:
   - do.validation :: TRUE, FALSE: DUMMY whether to do validation {da.val}
   - validation :: crossval: ('learntest' would also be an option, but is broken) req: do.validation == TRUE
*** classif.geoDA
**** Variable Parameters:
   - do.validation :: TRUE, FALSE: DUMMY whether to do validation {da.val}
   - validation :: crossval: ('learntest' would also be an option, but is broken) req: do.validation == TRUE
...
**** Variable Parameters:
   ...
   - K :: int 1..100 exp: {knn.k} number of NN to use
**** Default Parameters:
   - X :: FALSE: don't optimize, /we/ are doing that already
   - E :: FALSE: no effect when X not given.
...
**** Variable Parameters:
   ...
   - mincriterion.AMLRFIX1 :: 0.1..1: value of test statistics that must be exceeded to implement split. req: testtype == "Teststatistic"
...
**** Variable Parameters:
   - C :: 0.1..0.9: {tree.c} pruning confidence: req: R == FALSE
   - M :: int 1..32 exp: {tree.m} minimum instances per leaf
...
**** Variable Parameters:
   - currentlyNotImplemented :: MANUAL
...
- xwar.wt :: NULL: even splitting prob. MLR<=2.8
- forest :: TRUE: return forest object for prediction (duh) MLR<=2.8
**** Variable Parameters:
   - ntree :: int 100..4000 exp: {rf.numtree} number of trees to grow
   - mtry :: int 3..40: {rf.mtry} number of sampled variables
   - nodesize :: int 1..32 exp: {rf.nodesize} min samples per node
...
**** Changed (fixed) Parameters:
   ...
   - na.action :: na.omit: TODO this sucks, it is the only value both in mlr 2.8 and on github. imputation, if it happens, should happen during preprocessing.
...
- random ferns
**** Variable Parameters:
   - depth :: int 1..14 exp: depth of ferns. actually up to 16 is possible but puts lots of strain on memory.
...
- random forest + pca
**** Variable Parameters:
   - K :: int 2..40: MANUAL{sp("K", "int", c(2, 40), trafo=function(x) max(1, round(sum(info$n.feat) / x)))} number of variables per subset. Need to implement K = round(ncol(x) / Kinverse, 0).
...
**** Variable Parameters:
   ...
   - mfinal :: int 25..400 exp: number of boosting iterations
...
**** Default Parameters:
   ...
   - train.fraction :: 1
   - verbose :: FALSE
   - keep.data :: TRUE: TODO: as soon as mlr has this fixed, this must change.
*** classif.glmboost
**** Variable Parameters:
   - family :: AdaExp, Binomial
   - mstop :: int 25..400 exp: used to require m == "mstop", but that is the default
   - nu :: .001..0.3 exp
   - risk :: inbag, oobag, none: used to require m != "aic"
...
**** Default Parameters:
   ...
   - early_stopping_rounds :: 1: we don't want to use the early stopping feature, but passing NULL seems to be not an option.
...
**** Variable Parameters:
   ...
   - kernel :: vanilladot, polydot, rbfdot, tanhdot, laplacedot, besseldot, anovadot, splinedot: {svm.kernel} TODO: "matrix" is also available in principle
   - C :: .125..8 exp: {svm.c} constraint violation cost. req: type %in% c("C-svc", "C-bsvc", "spoc-svc", "kbb-svc")
   - nu :: .001..0.1 exp: {svm.nu} req: type == "nu-svc"
   - epsilon :: .001..0.5 exp: insensitive loss epsilon. req: type %in% c("eps-svr", "nu-svr", "eps-bsvm")
...
**** Variable Parameters:
   ...
   - tau :: .001..0.100 exp: regularization parameter
...
**** Variable Parameters:
   ...
   - nu :: .001..0.1 exp: {svm.nu} req: type == "nu-classification"
...
**** Variable Parameters:
   ...
   - hidden.AMLRFIX1 :: int 3..100 exp: len(5) {nn.h5} req: numlayers==5
   - hidden.AMLRFIX2 :: int 3..100 exp: len(7) {nn.h7} req: numlayers==7
...
**** Variable Parameters:
   ...
   - MaxNWts :: 100000: maximum number of weights. Maybe change this to abort slow runs prematurely. INJECT since mlr doesn't know about it.
**** Default Parameters:
...
**** Variable Parameters:
   ...
   - hidden.AMLRFIX1 :: int 3..100 exp: len(5) {nn.h5} req: numlayers==5
   - hidden.AMLRFIX2 :: int 3..100 exp: len(7) {nn.h7} req: numlayers==7
   - threshold :: .0001..0.1 exp: stopping criterion
...
**** Default Parameters:
   ...
   - keep.data :: TRUE: TODO: this must change when mlr fixes it on their end
**** Changed (fixed) Parameters:
