...
It is highly recommended to install my fork of the ~e1071~ and ~kernlab~ packages, which fix bugs that otherwise regularly lead to R crashing or hanging:
#+BEGIN_SRC R
...
This happens if you use ~automlr~ when the "fork" backend when you have already loaded a package that uses java. Restart your R session and prevent java-using packages from being loaded, e.g. by 
* Project status
...
- [-] release 0.2
  ...
  - [-] tests
    ...
    - [X] differentiate expensive tests from fast tests
  ...
  - [X] learner error imputation wrapper
- [ ] release 0.3
